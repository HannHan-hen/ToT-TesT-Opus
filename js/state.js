...
// Crop catalogue. `days` is grow time, `sell` the produce price at the
// shipping crate, `seed` the buy price at Marigold's. `stageW` is the draw
// width per growth stage (0..3), matching the turnip art's footprint.
export const CROPS = {
  radish: { name: "radish", days: 2, sell: 6, seed: 4, stageW: [30, 60, 64, 62] },
  turnip: { name: "turnip", days: 3, sell: 8, seed: 5, stageW: [34, 73, 75, 73] },
  carrot: { name: "carrot", days: 4, sell: 13, seed: 9, stageW: [30, 62, 68, 74] },
};
...
export const state = {
  ...
  inv: { radish: 0, turnip: 0, carrot: 0 }, // harvested produce, sold at the crate
  selectedSeed: "turnip",
...
export function load() {
  try {
    ...
    if (s && s.v === 2) {
      Object.assign(state, {
        day: s.day, coins: s.coins, seeds: s.seeds, inv: s.inv,
        selectedSeed: s.selectedSeed, crops: s.crops,
